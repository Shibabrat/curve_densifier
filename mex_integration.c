...
//     int numPts, numDim;
    int i,j;
    double *currPos, *iterPos, *timeSpan, *flag, *paramPr;
    mwSize dimen[2];
    size_t m,n;
//     double params = 0.1;
...
/* verify the number of input and output */
    ...
    /* 1st argument of time span is row vector of initial and final time */
    if(mxGetM(prhs[0])!=1) {
    ...
    /* 2nd argument of grid points is an array of size #pts x #dim */
    if(mxGetM(prhs[1])!= m){
...
/*  Get the pointer to the parameter*/
    paramPr = mxGetPr(params);
    double direction = paramPr[0];
...
//         mexPrintf("%lf \t %lf \n",currPt[0],currPt[1] );
        evolve_pt(currT, tau, currPt, iterPt, direction);
//         evolve_pt_event(currT, tau, currPt, iterPt, direction);
        iterPos[i] = iterPt[0];
