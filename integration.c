...
int ship_roll_model(double t, const double posIn[2], double velOut[2], void *params){
	...
	int dirtn = *(double *)params;
    
    double Mt, phi, pPhi;
    double phiCritical = 0.88;  /* Roll angle of vanishing stability */
//     double H = 0;
//     double H = 3.278734500288605;       // Fig 5c, case C 
//     double H = 4.918101750432908;        // Fig 5c, case D
//     double H = 6.557469000577210;       // Fig 5c, case E
//     double H = 8.196836250721512;       // Fig 5c, case F
    double H = 9.836203500865816;       //Fig 5c, case G
    double chi = 90*(pi/180);
    
    
    /* Disturbance due to regular waves */
    double alpha0 = 0.73, omegaN = 0.62, omegaE = 0.527, lambda = 221.94;
    
    /* Irregular waves*/
    Mt = 0;//I*alpha0*pow(omegaN,2)*pi*(H/lambda)*(sin(chi))*sin(omegaE*t);
	// mexPrintf("%lf\n",Mt);

    phi = posIn[0]; 
    pPhi = posIn[1];

    /* Parameters for Edith Terkol, with damping */
...
//             c5*pow(phi,5) + Mt/I);
    ...
    double HBar = alpha0*pi*(H/lambda);
    double omegaBar = omegaE/omegaN;
    double b1bar =  b1/omegaN, b2bar = b2, c2bar = c2/pow(omegaN,2), 
        c3bar = c3/pow(omegaN,2), c4bar = c4/pow(omegaN,2), 
            c5bar = c5/pow(omegaN,2);

    velOut[0] = pPhi;
    velOut[1] = (-b1bar*pPhi - b2bar*fabs(pPhi)*pPhi - phi - 
            c2bar*fabs(phi)*phi - c3bar*pow(phi,3) -c4bar*fabs(phi)*pow(phi,3) - 
            c5bar*pow(phi,5) + HBar*sin(omegaBar*t));
...
int duffing(double t, const double posIn[2], double velOut[2], void *params){
...
//     printf("%lf\n",epsilon);
    ...
    velOut[0] = x2;
    velOut[1] = x1 - pow(x1,3) + epsilon*sin(t);
...

{
    ...
    gsl_odeiv_control *c = gsl_odeiv_control_y_new(1e-14, 1e-14);
...
//     gsl_odeiv_system sys = {duffing, NULL, NDIM, &params};
...

{
	...
	double phiCritical = 0.88;  // angle of vanishing stability
